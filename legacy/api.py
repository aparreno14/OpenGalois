...
def _parse_coeffs_degree5(coeffs: Sequence[Any]) -> list[Fraction]:
    if len(coeffs) != 6:
        raise ValueError("Expected 6 coefficients [a5,a4,a3,a2,a1,a0] for degree-5 polynomial.")
    out = [_to_fraction(c) for c in coeffs]
    if out[0] == 0:
        raise ValueError("Leading coefficient a5 must be non-zero.")
    return out
...
def analyze(
    ...
    *,
    ...
    """Analyze a degree-5 polynomial and return a result plus certificate payload."""
    if prime_budget <= 0:
    ...
    coeffs = _parse_coeffs_degree5(polynomial)
    ...
    res = cert.get("result", {})
    status_raw = res.get("status", Status.unclassified.value)
    gg_raw = res.get("galois_group", GaloisGroup.UNKNOWN.value)
    sbr_raw = res.get("solvable_by_radicals", None)
    ...
    return Result(
        status=status,                  # si Status es Literal, esto encaja
