# Rule: `galois_group.QQ.reducible.quadratic_cubic.S3@1`
...
When the cubic discriminant is not a square, the cubic side is in the `S3` branch. The total group
remains `S3` exactly when the quadratic field `Q(√d1)` coincides with the unique quadratic subfield of
the cubic splitting field, equivalently when `d1*d2` is a square in `Q`.
...
4. Require a verified premise `IsSquareQQ(w)`.
...
8. Decode `G` canonically as `GroupId` with `system="smallgroup"` and require `(G.order, G.index) = (6, 2)`.
9. Accept.
