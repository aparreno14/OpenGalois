# Rule: `galois_group.QQ.reducible.quadratic_cubic.D6@2`
...
Galois group `D6` exactly when `d1*d2` is not a square in `Q`.
...
6. Require a verified premise `NonSquareQQ(w)` where `w = d1*d2` exactly in `Q`.
7. Decode `G` canonically as `GroupId(system="smallgroup")` and require `(G.order,G.index)=(12,4)`.
8. Accept.
