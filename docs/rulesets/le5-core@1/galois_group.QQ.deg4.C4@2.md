# Rule: `galois_group.QQ.deg4.C4@2`
...
These are the same premises as in `galois_group.QQ.deg4.C4@1`, except that the resolvent family required in the `ResolventQQ` premise is now the pair-sums family
...
This rule certifies the cyclic case by requiring both `IsSquareQQ` premises for these two exact values.
...
13. Recompute
    ...
    w_2=((b-s_0)^2-4d)\Delta.
    \]
14. Require both `IsSquareQQ` premises to bind to the exact `RatQQ` objects encoding `w1` and `w2`.
...
Examples adapted from Conrad's Table 9 (`Z/4Z` rows), now expressed in the pair-sums coordinate:
...
- `X^4 + 5X + 5`, with
  ...
  (-4s_0)\Delta = 550^2,
  \qquad
  ((-s_0)^2-4d)\Delta = 275^2.
  \]

Files:
- OK:
  - `fixtures/v3/le5-core@1/ok/galois_group.QQ.deg4.C4@2_001.json`
- BAD:
  - `fixtures/v3/le5-core@1/bad/galois_group.QQ.deg4.C4@2_fail_001.json`
