# Rule: `galois_group.QQ.deg4.A4@1`
...
Here the premise `DiscSquareQQ(f)` asserts that the discriminant of `f` **is** a square in `Q`.
Therefore `Gal(f)` is contained in `A4`.
...
by both 4 and 3, leaving only `A4` and `S4`. The square discriminant then forces containment in `A4`,
so the group must be `A4`.
...
3. Check there is a verified premise `DiscSquareQQ(f)` bound to the same `f` as in the claim.
...
it checks that the precise premises corresponding to Conrad's quartic `A4` criterion are present and
...
## 8) Fixtures
- OK:
  - `fixtures/v3/le5-core@1/ok/galois_group.QQ.deg4.A4@1_001.json`
- BAD:
  - `fixtures/v3/le5-core@1/bad/galois_group.QQ.deg4.A4@1_fail_001.json`
  - `fixtures/v3/le5-core@1/bad/galois_group.QQ.deg4.A4@1_fail_002.json`
