...
This rule is computational (`recompute-and-compare`): the verifier monicizes the quartic internally,
recomputes the cubic resolvent by the explicit quartic formula, and checks exact equality.
...
- `E_SIDE_CONDITION` — the degree premise is not `4`, or the decoded polynomial is not quartic.
- `E_P_MISMATCH` — `p` is not the fixed canonical `MPolyQQ` for `x1x2 + x3x4`.
...
## 7) Fixtures
- OK: `fixtures/v3/le5-core@1/ok/resolvent.QQ.compute.deg4.cubic_x1x2_plus_x3x4@1_001.json`
- BAD: `fixtures/v3/le5-core@1/bad/resolvent.QQ.compute.deg4.cubic_x1x2_plus_x3x4@1_fail_001.json`
