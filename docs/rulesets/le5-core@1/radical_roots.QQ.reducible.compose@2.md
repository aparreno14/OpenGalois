...
for a reducible polynomial whose factor references already have certified radical root lists.
...
5. For each such factor `g`, require a verified premise `RadicalRoots(g, roots_g)`.
6. Recompute the expected root list for `f` by concatenating the already certified lists `roots_g` in the exact order of factor occurrences in `factors.items`.
...
- `E_SIDE_CONDITION` — the certified factorization is not reducible (has fewer than two factors in `factors.items`).
- `E_MISMATCH` — the claimed radical root list does not equal the canonical concatenation of factor-level lists.

## 8) Fixtures
- OK:
  - `fixtures/v3/le5-core@1/ok/radical_roots.QQ.reducible.compose@2_001.json`
- BAD:
  - `fixtures/v3/le5-core@1/bad/radical_roots.QQ.reducible.compose@2_fail_001.json`
