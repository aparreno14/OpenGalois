# Rule: `galois_group.QQ.reducible.double_quadratic.V4@1`
...
with the intended conclusion that `G` is the Klein four group `V4`.
...
- `IrreducibleQQ(q2)`.
- `Discriminant(q2, d2)`.
- `NonSquareQQ(c)` where `c` is the rational number `d1*d2`.
...
As in the `C2` rule, the linear factors contribute only rational roots, and repeated irreducible
factors do not alter the splitting field. Thus the splitting field of `f` is the same as the
splitting field of the square-free quadratic core `q1 q2`.

If `d1 = disc(q1)` and `d2 = disc(q2)`, then the quadratic fields are equal iff `d1*d2` is a square.
In the present branch `d1*d2` is **not** a square, so the two quadratic fields are distinct and their
compositum is biquadratic. Hence

\[
\operatorname{Gal}(f/\mathbf{Q}) \cong C_2 	imes C_2 \cong V_4.
\]
...
8. Require a verified premise `NonSquareQQ(c)` whose argument `c` is exactly the canonical `RatQQ`
   object encoding that product.
9. Decode `G` canonically as `GroupId` with `system="smallgroup"` and require `(G.order, G.index) = (4,2)`.
10. Accept.
...
- `E_GROUP_MISMATCH` — the claim does not state `V4`.
