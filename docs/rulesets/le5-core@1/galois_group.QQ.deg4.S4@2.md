# Rule: `galois_group.QQ.deg4.S4@2`
...
These are the same premises as in `galois_group.QQ.deg4.S4@1`, except that the resolvent family required in the `ResolventQQ` premise is now the pair-sums family
...
Here the premise `DiscNonSquareQQ(f)` asserts that the discriminant of `f` is not a square in `Q`. Therefore the discriminant branch forces the conclusion
...
## 8) Fixtures
- OK:
  - `fixtures/v3/le5-core@1/ok/galois_group.QQ.deg4.S4@2_001.json`
- BAD:
  - `fixtures/v3/le5-core@1/bad/galois_group.QQ.deg4.S4@2_fail_001.json`
