...
EXHAUSTIVE_DEG3_BOUND = int(os.getenv("OPENGALOIS_SYMPY_DEG3_BOUND", "5"))
EXHAUSTIVE_DEG4_BOUND = int(os.getenv("OPENGALOIS_SYMPY_DEG4_BOUND", "3"))
RANDOM_DEG3_CASES = int(os.getenv("OPENGALOIS_SYMPY_RANDOM_DEG3", "200"))
RANDOM_DEG4_CASES = int(os.getenv("OPENGALOIS_SYMPY_RANDOM_DEG4", "200"))
RANDOM_COEFF_BOUND = int(os.getenv("OPENGALOIS_SYMPY_RANDOM_BOUND", "20"))
ROOT_SAMPLE_PER_DEGREE = int(os.getenv("OPENGALOIS_SYMPY_ROOT_SAMPLE", "20"))
ROOT_PRECISION = int(os.getenv("OPENGALOIS_SYMPY_ROOT_PRECISION", "80"))
ROOT_TOL = sp.Float(os.getenv("OPENGALOIS_SYMPY_ROOT_TOL", "1e-25"), ROOT_PRECISION)

# Canonical small cases covering all irreducible degree-3 and degree-4 branches.
FIXED_CASES: tuple[tuple[tuple[int, ...], str], ...] = (
    ((1, 0, -3, 1), "C3"),       # x^3 - 3*x + 1
    ((1, 1, 0, 1), "S3"),        # x^3 + x^2 + 1
    ((1, 0, 0, -1, -1), "S4"),   # x^4 - x - 1
    ((1, 0, 0, 8, 12), "A4"),    # x^4 + 8*x + 12
    ((1, 0, 0, 36, 63), "V4"),   # x^4 + 36*x + 63
    ((1, 0, 0, 5, 5), "C4"),     # x^4 + 5*x + 5
    ((1, 0, 0, 3, 3), "D4"),     # x^4 + 3*x + 3
)
...
def _is_valid_irreducible_case(coeffs: Sequence[int]) -> bool:
    poly = _sympy_poly_from_coeffs(coeffs)
    return bool(poly.is_irreducible and poly.discriminant() != 0)


def _monic_integer_polys(degree: int, *, bound: int) -> Iterator[tuple[int, ...]]:
    for tail in itertools.product(range(-bound, bound + 1), repeat=degree):
        yield (1, *tail)


def _random_irreducible_cases(
    degree: int,
    *,
    count: int,
    bound: int,
    ...
    while len(cases) < count and attempts < max_attempts:
        attempts += 1
        coeffs = (1, *(rng.randint(-bound, bound) for _ in range(degree)))
        if coeffs in seen:
            continue
        seen.add(coeffs)
        if _is_valid_irreducible_case(coeffs):
            cases.append(coeffs)

    if len(cases) < count:
        raise AssertionError(
            f"Could only generate {len(cases)} irreducible degree-{degree} cases "
...
def _sympy_group_alias(poly: Any) -> str:
    ...
    if degree == 4:
        ...
        if order == 8:
            return "D4"
        if order == 4:
            element_orders = {int(elt.order()) for elt in group.generate_schreier_sims()}
            return "C4" if 4 in element_orders else "V4"

    raise AssertionError(
        f"unsupported SymPy Galois group: degree={degree}, order={order}, group={group}"
    )
...
def _root_multiset_distance(left: Sequence[Any], right: Sequence[Any]) -> Any:
    ...
    for perm in itertools.permutations(right):
        current = max(sp.Abs(sp.N(a - b, ROOT_PRECISION)) for a, b in zip(left, perm, strict=True))
...
def _assert_matches_sympy(coeffs: Sequence[int], *, check_roots: bool) -> None:
    ...
    sp_group = _sympy_group_alias(poly)
...
def test_sympy_fixed_groups_and_roots() -> None:
    for coeffs, expected_group in FIXED_CASES:
        poly = _sympy_poly_from_coeffs(coeffs)
        assert _sympy_group_alias(poly) == expected_group
        _assert_matches_sympy(coeffs, check_roots=True)


def test_sympy_exhaustive_small_groups() -> None:
    checked = 0
    for degree, bound in ((3, EXHAUSTIVE_DEG3_BOUND), (4, EXHAUSTIVE_DEG4_BOUND)):
        for coeffs in _monic_integer_polys(degree, bound=bound):
            if _is_valid_irreducible_case(coeffs):
                _assert_matches_sympy(coeffs, check_roots=False)
                checked += 1
    assert checked > 0


def test_sympy_random_groups_and_sampled_roots() -> None:
    degree3_cases = _random_irreducible_cases(
        3,
        count=RANDOM_DEG3_CASES,
        bound=RANDOM_COEFF_BOUND,
        seed=20260624,
    )
    degree4_cases = _random_irreducible_cases(
        4,
        count=RANDOM_DEG4_CASES,
        bound=RANDOM_COEFF_BOUND,
        seed=20260625,
    )

    for coeffs in itertools.chain(degree3_cases, degree4_cases):
        _assert_matches_sympy(coeffs, check_roots=False)

    sampled_cases = degree3_cases[:ROOT_SAMPLE_PER_DEGREE] + degree4_cases[:ROOT_SAMPLE_PER_DEGREE]
    for coeffs in sampled_cases:
        _assert_matches_sympy(coeffs, check_roots=True)
